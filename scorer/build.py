...
def read_version (CMakeLists):
  ...
  with open(CMakeLists, 'r') as fp:
    ...
    for row in fp:
      ...
      if 'SCORER_MAJOR' in row:
        version.append(row.split('SCORER_MAJOR')[-1])

      elif 'SCORER_MINOR' in row:
        version.append(row.split('SCORER_MINOR')[-1])

      elif 'SCORER_REVISION' in row:
        version.append(row.split('SCORER_REVISION')[-1])

...
if [ "$build_type" == "" ]; then
  ...
  cd build_release
  cmake .. -DCMAKE_BUILD_TYPE="Release" -DOMP=$allow_omp $other
  cd ..

  rm -rf build_debug
  mkdir -p build_debug
  cd build_debug
  cmake .. -DCMAKE_BUILD_TYPE="Debug" -DOMP=$allow_omp $other
  cd ..
  ...
  cmake .. -DCMAKE_BUILD_TYPE=$build_type -DOMP=$allow_omp $other
